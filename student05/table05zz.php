...
function createTable($mysqli)
{
    ...
    if(!$id)
    {
	    $sql = "CREATE TABLE table05zz (id INT NOT NULL AUTO_INCREMENT,PRIMARY KEY( id ),";
	    $sql .= "type VARCHAR(20),";
	    $sql .= "brand VARCHAR(30),";
	    $sql .= "powertype VARCHAR(20),";
	    $sql .= "condition VARCHAR(30),";
	    $sql .= "location VARCHAR(20),";
...
function showList($mysqli, $msg)
{
	...
	echo 	'<div class="col-md-12">
			<form action="table05zz.php" method="POST">
			...
			<h2 style="color: white;">Power Tools Database</h2></td></tr>
			<tr style="font-weight:800; font-size:20px;"><td>ID #</td><td>Type</td><td>Brand</td>
			<td>Power Type</td><td>Condition</td><td></td><td>Location</td><td>Price</td><td>Description</td><td></td></tr>';
...
function showInsertForm ()
{
	...
	<form name="basic" method="POST" action="table05zz.php" onSubmit="return validate();">
		<table class="table table-condensed" style="border: 1px solid #dddddd; border-radius: 5px; box-shadow: 2px 2px 10px;">
			...
			<tr><td>Power Type: </td><td><input type="edit" name="powertype" value="" size="30"></td></tr>
			<tr><td>Color: </td><td><input type="edit" name="condition" value="" size="20"></td></tr>
			<tr><td>String/Wind: </td><td><input type="edit" name="location" value="" size="30"></td></tr>
		...
		</table><a href="table05zz.php" class="btn btn-primary">Display Database</a></form></div>';
}
...
function showUpdateForm($mysqli) 
{
	...
	if($result = $mysqli->query("SELECT * FROM $usertable WHERE id = $index"))
	{
		while($row = $result->fetch_row())
		{
			echo '	<br>
					<div class="col-md-4">
					<form name="basic" method="POST" action="table05zz.php">
						<table class="table table-condensed" style="border: 1px solid #dddddd; border-radius: 5px; box-shadow: 2px 2px 10px;">
							...
							<tr><td>Power Type: </td><td><input type="edit" name="powertype" value="' . $row[3] . '" size="30"></td></tr>
							<tr><td>Condition: </td><td><input type="edit" name="condition" value="' . $row[4] . '" size="20"></td></tr>
							<tr><td>Location: </td><td><input type="edit" name="location" value="' . $row[5] . '" size="30"></td></tr>
...
{
    global $type, $brand, $powertype, $condition, $location, $price, $descript, $usertable;
    
    $stmt = $mysqli->stmt_init();
    if($stmt = $mysqli->prepare("INSERT INTO $usertable (type,brand,powertype,condition,location,price,descript) VALUES (?, ?, ?, ?, ?, ?, ?)"))
    {
        // Bind parameters. Types: s = string, i = integer, d = double,  b = blob, etc.
		// bind_param replaces question mark(s) with contents of variable(s) to protect against sql injection
        $stmt->bind_param('sssssss', $type, $brand, $powertype, $condition, $location, $price, $descript);
        $stmt->execute();
        $stmt->close();
    }
}
...
function updateRecord($mysqli)
{
    ...
    $stmt = $mysqli->stmt_init();
    if($stmt = $mysqli->prepare("UPDATE $usertable SET type = ?, brand = ?, powertype = ?, condition = ?, location = ?, price = ?, descript = ? WHERE id = ?"))
    {
            // Bind parameters. Types: s = string, i = integer, d = double,  b = blob, etc.
			// bind_param replaces question mark(s) with contents of variable(s) to protect against sql injection
            $stmt->bind_param('sssssssi', $type, $brand, $powertype, $condition, $location, $price, $descript, $index);
            $stmt->execute();
            $stmt->close();
    }
}
