<?php
/**
 * This file displays a HTML page for adding entries to a database table
 *
...
function insertRecord($mysqli)
{
    ...
    global $title, $author, $publisher, $published, $genre, $isbn, $usertable;
    
    /* Initialise the statement. */
    $stmt = $mysqli->stmt_init();
    /* Notice the two ? in values, these will be bound parameters*/
    if($stmt = $mysqli->prepare("INSERT INTO $usertable (title, author, publisher, published, genre, isbn) VALUES (?, ?, ?, ?, ?, ?)"))
    {
            /* Bind parameters. Types: s = string, i = integer, d = double,  b = blob, etc... */
            $stmt->bind_param('ssssss', $title, $author, $publisher, $published, $genre, $isbn);
...
function selectRecords($mysqli)
{
    ...
    if($stmt = $mysqli->prepare("select * from $usertable"))
    {
        ...
        $stmt->execute();
        /* bind result vars */
        $stmt->bind_result($id, $title, $author, $publisher, $published, $genre, $isbn);
				...
				<form name='basic' method='Post' action='table06.php'>
					<table border=0 cellpadding=5 cellspacing=0>
						...
						<tr>
							<td>Published</td>
							<td>
								<input type='text' name='published' size=20>
							</td>
						</tr>
						<tr>
							<td>Genre</td>
...
{
    echo "<br><form name='basic' method='Post' action='table06.php'>
...
function createTable($mysqli)
{
    ...
    if(!$id)
    {
		...
		$sql .= "published VARCHAR(30)";
		$sql .= "genre VARCHAR(30)";
		$sql .= "isbn VARCHAR(30)";
		$sql .= "author VARCHAR(30)";
		$sql .= "PRIMARY KEY ( id ));";
