		...
		if ( isset( $_POST['action'] ) ) {
			...
			switch ( $_POST['action'] ) {
				case 'add':
					...
					echo '<script>window.location.replace("http://cis355.com/student06/table06.php");</script>';
					break;

				case 'delete':
					if ( delete_record( $database, 'table06' ) )
						display_msg('Record #' . $_POST['book_id'] . ' was sucessfully removed from the table.' );
					else
						display_error( 'ERROR: Deleting record' . $_POST['book_id'] . 'from the database.'  );
					break;
...
function insert_record( $database, $table ) {
    ...
    if($stmt = $database->prepare("INSERT INTO $table (location_id, post_author, post_date, book_author, book_title, book_publisher, book_published, book_edition, book_genre, book_isbn, book_price, book_description, book_condition) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"))
    {
		/* Bind parameters. Types: s = string, i = integer, d = double,  b = blob, etc... */
		$stmt->bind_param('iisssssisssss', $location_id, $post_author, date( 'Y-m-d', strtotime( $post_date ) ), $book_author, $book_title, $book_publisher, date( 'Y-m-d', strtotime( $book_published ) ), $book_edition, $book_genre, $book_isbn, floatval( $book_price ), $book_description, $book_condition );
...
function update_record( $database, $table ) {
    ...
    $stmt = $database->stmt_init();
    /* Notice the two ? in values, these will be bound parameters */
    if($stmt = $database->prepare( "UPDATE $table SET location_id=?, post_author=?, post_date=?, book_author=?, book_title=?, book_publisher=?, book_published=?, book_edition=?, book_genre=?, book_isbn=?, book_price=?, book_description=?, book_condition=? WHERE ID=?" ) )
    {
            /* Bind parameters. Types: s = string, i = integer, d = double,  b = blob, etc... */
            $stmt->bind_param('iisssssisssssi', $location_id, $post_author, date( 'Y-m-d', strtotime( $post_date ) ), $book_author, $book_title, $book_publisher, date( 'Y-m-d', strtotime( $book_published ) ), $book_edition, $book_genre, $book_isbn, floatval( $book_price ), $book_description, $book_condition, intval( $book_id ) );
