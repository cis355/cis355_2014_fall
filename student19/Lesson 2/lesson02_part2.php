...
function insertRecord($mysqli)
{
    ...
    global $brand, $model, $price, $cond, $usertable;
    
    /* Initialise the statement. */
    $stmt = $mysqli->stmt_init();
    /* Notice the two ? in values, these will be bound parameters*/
    if($stmt = $mysqli->prepare("INSERT INTO $usertable(brand,model,price,cond) VALUES (?, ?, ?, ?)"))
    {
            /* Bind parameters. Types: s = string, i = integer, d = double,  b = blob, etc... */
            $stmt->bind_param('ssis', $brand,$model,$price,$cond);
...
function updateRecord($mysqli)
{
    ...
    $sql = "UPDATE $usertable WHERE brand=?";
...
function selectRecords($mysqli)
{
    ...
    if($stmt = $mysqli->prepare("select * from $usertable"))
    {
        ...
        $stmt->execute();
        /* bind result vars */
        $stmt->bind_result($id, $name, $model, $price, $cond);
        /* read while fetch returns data*/
        echo "<br> Data selected from the MySQL table.<table>
<tr><th>ID</th><th>Brand</th><th>Model</th><th>Price</th><th>Condition</th></tr>";
        while($stmt->fetch()){
                /* work with bound result vars */
                echo "<tr><td>$id</td><td>$brand</td><td>$model</td><td>$price</td><td>$cond</td></tr>";
        }
        echo "</table>";
        $stmt->close();
    }
...
function showForm()
{
    return "<!DOCTYPE html><html>
        ...
        <form name='basic' method='Post' action='lesson02_part2.php' onSubmit='return validate();'>
...
{
    echo "<br><form name='basic' method='Post' action='lesson02_part2.php' onSubmit='return validate();'>
