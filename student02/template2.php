...
function insertRecord($mysqli)
{
    ...
    if($stmt = $mysqli->prepare("INSERT INTO $usertable (name,descript) VALUES (?, ?)"))
    {
...
function updateRecord($mysqli)
{
	...
	/* trying to get a hold of the name */
			$username = $_GET['name'];
			$_POST['namename'];
			$username = $_POST['name'];
			if ($username == "letmein") {
				$sql = "UPDATE $usertable SET descript=? WHERE name=?";
			print ("Welcome back, friend!");
...
function showForm()
{
    return "<!DOCTYPE html><html>
        ...
        <form name='basic' method='Post' action='template2.php' onSubmit='return validate();'>
...
{
    echo "<br><form name='basic' method='Post' action='template2.php' onSubmit='return validate();'>
