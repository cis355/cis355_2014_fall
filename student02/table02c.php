...
course:
input:
processing: this program requires a browser user to call table02c.php
			from the 
...
if($mysqli) // if successful connection
{
	...
	if isset($_POST['submit'])) // submit means insert
	{
		// Create table if necessary
		createTable($mysqli);
		// 3. Show insert form
...
{
	echo 	'<div class="col-md-12"><form action="table02c.php" method="POST"><table class="table table-condensed" style="border: 1px solid #dddddd; border-radius: 5px; box-shadow: 2px 2px 10px;">
	...
	echo    '</table><input type="hidden" id="hid" name="hid" value=""><input type="hidden" id="uid" name="uid" value=""><input type="submit" name="submit" value="Add an Entry class ="btn btn-primary"><class="btn btn-primary">Add an Entry</a></div></form>';
...
function createTable($mysqli)
{
    ...
    if(!$id)
    {
	    $sql = "CREATE TABLE table02c (id INT NOT NULL AUTO_INCREMENT,PRIMARY KEY( id ),";
	    $sql .= "type VARCHAR(20),";
...
function insertForm ()
{
	...
	<form name="basic" method="POST" action="table02c.php" onSubmit="return validate();">
		...
		<a href="table02c.php" class="btn btn-primary">Display Database</a>
	</form>
</div>';
}