...
# --------------------------------------------------------------------------- #
# program: table02.php
# author:  Erik Andersson/george corser
...
function createTable($mysqli)
{
    ...
    if(!$id)
    {
	    $sql = "CREATE TABLE table02 (id INT NOT NULL AUTO_INCREMENT,PRIMARY KEY( id ),";
	    $sql .= "user_id INT,";
	    $sql .= "loc_id INT,";
	    $sql .= "amount_paid decimal(10,2),";
	    $sql .= "game_name VARCHAR(40),";
	    $sql .= "console_name VARCHAR(15),";
	    $sql .= "game_description VARCHAR(150),";
	    $sql .= "quality VARCHAR(15),";
		$sql .= "FOREIGN KEY (user_id) REFERENCES users02 (user_id),";
...
function showList($mysqli, $msg)
{
	...
	echo 	'<div class="col-md-12">
			<form action="table02.php" method="POST">
			...
			<h2 style="color: white;">Video Game Database (table02)</h2></td></tr>
			<tr style="font-weight:800; font-size:20px;"><td>ID #</td><td>user_id</td><td>loc_id</td>
			<td>amount_paid:</td><td>game_name:</td><td>console_name:</td><td>game_description</td><td>quality</td><td></td></tr>';
 ...
 echo'<a href="http://www.cis355.com/student02/ProjectBio.txt">Author Bio and Program Notes</a>'; 
}
...
function showInsertForm ($mysqli)
{
	...
	<form name="basic" method="POST" action="table02.php" onSubmit="return validate();">
		<table class="table table-condensed" style="border: 1px solid #dddddd; border-radius: 5px; box-shadow: 2px 2px 10px;">
            ...
            echo "<select class='form-control' name = 'loc_id' id='location'>";
				...
				echo '</td></tr>
			<tr><td>amount_paid: </td><td><input type="edit" name="amount_paid" value="" size="10"></td></tr>
			<tr><td>game_name: </td><td><input type="edit" name="game_name" value="" size="40"></td></tr>
		...
		</table><a href="table02.php" class="btn btn-primary">Display Database</a></form></div>';
}
...
function showUpdateForm($mysqli) 
{
	...
	if($result = $mysqli->query("SELECT * FROM $usertable WHERE id = $index"))
	{
		while($row = $result->fetch_row())
		{
			echo '	<br>
					<div class="col-md-4">
					<form name="basic" method="POST" action="table02.php">
						<table class="table table-condensed" style="border: 1px solid #dddddd; border-radius: 5px; box-shadow: 2px 2px 10px;">
							<tr><td colspan="2" style="text-align: center; border-radius: 5px; color: white; background-color:#333333;"><h2>Video Game Form</h2></td></tr>
            ...
            echo "<select class='form-control' name = 'loc_id' id='location'>";
				...
				echo '</td></tr>		
							<tr><td>amount_paid: </td><td><input type="edit" name="amount_paid" value="' . $row[3] . '" size="10"></td></tr>
							<tr><td>game_name: </td><td><input type="edit" name="game_name" value="' . $row[4] . '" size="40"></td></tr>
			...
			echo '	<br>
					...
					<form name="basic" method="POST" action="table02.php">
						<table class="table table-condensed" style="border: 1px solid #dddddd; border-radius: 5px; box-shadow: 2px 2px 10px;">
							...
							<tr><td>amount_paid: </td><td><input type="edit" name="amount_paid" value="' . $row[3] . '" size="10" readonly></td></tr>
							<tr><td>game_name: </td><td><input type="edit" name="game_name" value="' . $row[4] . '" size="40" readonly></td></tr>
							...
							<tr><td><input type="submit" name="viewCompleted" class="btn btn-primary" value="Back to Table"></td></tr>
						</table>
						<input type="hidden" name="uid" value="' . $row[0] . '">
					</form>
				</div>';
...
{
    global $user_id, $loc_id, $amount_paid, $game_name, $console_name, $game_description, $quality, $usertable;
    
    $stmt = $mysqli->stmt_init();
    if($stmt = $mysqli->prepare("INSERT INTO $usertable (user_id,loc_id,amount_paid,game_name,console_name,game_description,quality) VALUES (?, ?, ?, ?, ?, ?, ?)"))
    {
        // Bind parameters. Types: s = string, i = integer, d = double,  b = blob, etc.
		// bind_param replaces question mark(s) with contents of variable(s) to protect against sql injection
        $stmt->bind_param('iidssss', $user_id, $loc_id, $amount_paid, $game_name, $console_name, $game_description, $quality);
        $stmt->execute();
        $stmt->close();
    }
}
...
function updateRecord($mysqli)
{
    ...
    $stmt = $mysqli->stmt_init();
    if($stmt = $mysqli->prepare("UPDATE $usertable SET user_id = ?, loc_id = ?, amount_paid = ?, game_name = ?, console_name = ?, game_description = ?, quality = ? WHERE id = ?"))
    {
            // Bind parameters. Types: s = string, i = integer, d = double,  b = blob, etc.
			// bind_param replaces question mark(s) with contents of variable(s) to protect against sql injection
            $stmt->bind_param('iidssssi', $user_id, $loc_id, $amount_paid, $game_name, $console_name, $game_description, $quality, $index);
            $stmt->execute();
            $stmt->close();
    }
}
