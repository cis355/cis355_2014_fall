...
function showList($mysqli, $msg) 
{
	...
	echo 	'<div class="col-md-12">
			...
			<h2 style="color: white;">Musical Instrument Database (table14)</h2>
...
{
    global $type, $brand, $model, $color, $strWind, $price, $descript;
	global $usertable;
    
    $stmt = $mysqli->stmt_init();
    if($stmt = $mysqli->prepare("INSERT INTO $usertable (type,brand,model,color,
	    strWind,price,descript) VALUES (?, ?, ?, ?, ?, ?, ?)"))
    {
        // Bind parameters. Types: s=string, i=integer, d=double, etc.
		// protects against sql injections
        $stmt->bind_param('sssssss', $type, $brand, $model, $color, $strWind, 
		    $price, $descript);
        $stmt->execute();
        $stmt->close();
    }
...
function updateRecord($mysqli)
{
    ...
    if($stmt = $mysqli->prepare("UPDATE $usertable SET type = ?, brand = ?, 
	    model=?, color=?, strWind=?, price=?, descript=? WHERE id = ?"))
    {
        // Bind parameters. Types: s=string, i=integer, d=double, etc.
		// protects against sql injections
        $stmt->bind_param('sssssssi', $type, $brand, $model, $color, $strWind, 
