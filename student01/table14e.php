<?php
# --------------------------------------------------------------------------- #
# program: table14e.php
# author:  george corser
# course:  cis355 fall 2014
# purpose: template for cis355
#          to update this template, change all instances of "table14e.php" to your php filename
...
function showList($mysqli, $msg)
{
	...
	echo 	'<div class="col-md-12">
			<form action="table14e.php" method="POST">
	...
	if($result = $mysqli->query("SELECT * FROM $usertable"))
	{
		while($row = $result->fetch_row())
		{
			echo '<tr><td>' . $row[0] . '</td><td>' . $row[1] . '</td><td>' . $row[2] . '</td><td>' . $row[3] . 
				 ...
				 '</td><td><input name="deleteSelected" type="submit" class="btn btn-danger" value="Delete" onclick="setHid(' 
				 . $row[0] .')" /> 
				 <input style="margin-left: 10px;" type="submit" name="updateSelected" class="btn btn-primary" 
				 value="Update" onclick="setUid(' 
				 . $row[0] . ');" />';
		}
	}
	$result->close();
...
function showInsertForm ()
{
	...
	<form name="basic" method="POST" action="table14e.php" onSubmit="return validate();">
		<table class="table table-condensed" style="border: 1px solid #dddddd; border-radius: 5px; box-shadow: 2px 2px 10px;">
		...
		</table><a href="table14e.php" class="btn btn-primary">Display Database</a></form></div>';
}
...
function showUpdateForm($mysqli) 
{
	...
	if($result = $mysqli->query("SELECT * FROM $usertable WHERE id = $index"))
	{
		while($row = $result->fetch_row())
		{
			echo '	<br>
					<div class="col-md-4">
					<form name="basic" method="POST" action="table14e.php">
					<table class="table table-condensed" style="border: 1px solid #dddddd; 
						...
						<tr><td>Description: </td><td><textarea style="resize: none;" name="descript" cols="40" rows="3">' . $row[7] 
						. '</textarea></td></tr>
...
function insertRecord($mysqli)
{
	...
	/* gpc: fix duplicate error
	// check for duplicate
	$query = "SELECT COUNT(id) AS mycount FROM $usertable WHERE model = '".$model."' AND brand = '".$brand."'";
