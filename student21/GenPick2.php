...
//	if($_SESSION['id']== $row[9]){
            echo'
			   ...
			   <input type="submit" name="updateSelected" class="btn btn-primary" value="Update" onclick="setUid(' . $row[0] . ');" />
...
//	echo "You are logged in as user: ".$_SESSION["user"].
	...
	<form name="basic" method="POST" action="GenPick2.php" onSubmit="return validate();">
...
';
 ...
 echo '<tr><td>GeneratorModel: </td><td>';
            echo "<select class='form-control' name = 'GenModel' id='GenModel'>";
				if($sql_statement = $mysqli->query("SELECT * FROM GenModel")){
                  while($loc_row = $sql_statement->fetch_object()){
                    if($loc_row->GenModel == $userlocation){
                      echo"<option value='".$loc_row->GenModel. 
...
echo'
   	...
   	<tr><td><input type="submit" name="insertCompleted" class="btn btn-success" value="Add Entry"></td>
		...
		</table><a href="GenPick2.php" class="btn btn-primary">Display Database</a></form></div>';
}
