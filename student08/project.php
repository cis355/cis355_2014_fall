...
#          instructions: change all "project.html" to your php filename, and 
...

{
...

    {
		...
		$sql .= "FOREIGN KEY (division_id) REFERENCES locations (division_id),";
...

{
	...
	echo 	'<div class="col-md-12">
			...
			<td>User</td><td>Division</td><td></td></tr>';
...

{
	...
	<form name="basic" method="POST" action="project.html" 
		...
		<table class="table table-condensed" style="border: 1px solid #dddddd; 
			...
			<tr><td>Height: </td><td><input type="edit" name="height" value="" 
			...
			echo '</td></tr>
			    ...
			    <tr><td>Division ID: </td><td><textarea style="resize: none;" 

			    name="division_id" cols="40" rows="3"></textarea></td></tr>
		        ...
		        </table><a href="project.html" class="btn btn-primary">
			...
			echo '	<br>
					...
					<form name="basic" method="POST" action="project.html">
						...
						<table class="table table-condensed" 
							...
							<tr><td>Height: </td><td><input type="edit" 
							...
							<tr><td>Division ID: </td><td><textarea 							

							style="resize: none;" name="division_id" cols="40" 							
...

{
    ...
    global $name, $height, $weight, $hometown, $finisher, $price, $description, $user_id, $division_id ;
    ...
    if($stmt = $mysqli->prepare("INSERT INTO $usertable (type,height,weight,hometown,

	    finisher,price,description,user_id,division_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)"))

    {
        ...
        $stmt->bind_param('sssssssii', $name, $height, $weight, $hometown, $finisher, 

		    $price, $description, $user_id, $division_id);
...

{
    ...
    if($stmt = $mysqli->prepare("UPDATE $usertable SET type = ?, height = ?, 

	    weight=?, hometown=?, finisher=?, price=?, description=?, user_id=?, division_id=? WHERE id = ?"))

    {
        ...
        $stmt->bind_param('sssssssiii', $name, $height, $weight, $hometown, $finisher, 

		    $price, $description, $user_id, $division_id, $index);
...
echo '<br><br>  <a href="http://cis355.com/student08/Bio08.html">About This Table and Author!</a>  </div>';
