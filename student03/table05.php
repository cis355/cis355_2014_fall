...
#          instructions: change all "table05.php" to your php filename, and 
...
function createTable($mysqli)
{
    ...
    if(!$id)
    {
	    $sql = "CREATE TABLE table05 
		       (id INT NOT NULL AUTO_INCREMENT,PRIMARY KEY( id ),";
	    $sql .= "toolkind VARCHAR(20),";
	    $sql .= "brand VARCHAR(30),";
	    $sql .= "powerkind VARCHAR(20),";
	    $sql .= "color VARCHAR(30),";
	    $sql .= "hascase VARCHAR(20),";
...
function showList($mysqli, $msg) 
{
	...
	echo 	'<div class="col-md-12">
			...
			<tr><td colspan="12" style="text-align: center; border-radius: 5px; 
			color: green; background-color:#333333;">
			<h2 style="color: orange;">Table 05 (Power Tools)</h2>
			</td></tr><tr style="font-weight:800; font-size:20px;">
			<td>ID</td><td>Tool Type</td><td>Brand</td>
			<td>Power Type</td><td>Color</td><td>Had Case?</td>
			<td>Price</td><td>Description</td>						
			<td>Location</td><td>User</td><td></td></tr>';
	...
	{
	  // display html buttons 
	  echo  '<input type="submit" name="insertSelected" value="Add an Entry" 
...
function showInsertForm ($mysqli)
{
	...
	<form name="basic" method="POST" action="table05.php" 
		...
		<table class="table table-condensed" style="border: 1px solid #dddddd; 
			...
			<tr><td>Tool Type: </td><td><input type="edit" name="toolkind" value="" 
			size="20"></td></tr>
			<tr><td>Brand: </td><td><input type="edit" name="brand" value="" 
			size="20"></td></tr>
			<tr><td>Power Type: </td><td><input type="edit" name="powerkind" value="" 
			size="30"></td></tr>
			<tr><td>Color: </td><td><input type="edit" name="color" value="" 
			size="20"></td></tr>
			<tr><td>Had case?: </td><td><input type="edit" name="hascase" 
			value="" size="30"></td></tr>
...
*/	
			...
			echo '</td></tr>
			    <tr><td>User: </td><td><input type="edit" name="user_id" value="'.$_SESSION["id"].'" 
			            size="20" readonly></td></tr>
		        ...
		        </table><a href="table05.php" class="btn btn-primary">
			...
			echo '	<br>
					...
					<form name="basic" method="POST" action="table05.php">
						<table class="table table-condensed" 
							...
							<h2>Power Tools Form</h2></td></tr>
							<tr><td>Tool Type: </td><td><input type="edit" 
							name="toolkind" value="'. $row[1] .'" size="20">
							</td></tr>
							<tr><td>Brand: </td><td><input type="edit" 
							name="brand" value="' . $row[2] . '" size="20">
							</td></tr>
							<tr><td>Power Type: </td><td><input type="edit" 
							name="powerkind" value="' . $row[3] . '" size="30">
							</td></tr>
							<tr><td>Color: </td><td><input type="edit" 
							name="color" value="' . $row[4] . '" size="20">
							</td></tr>
							<tr><td>Had Case?: </td><td><input type="edit" 
							name="hascase" value="' . $row[5] . '" size="30">
							</td></tr>
...
{
    global $toolkind, $brand, $powerkind, $color, $hascase, $price, $descript, $location_id, $user_id ;
	global $usertable;
    
    $stmt = $mysqli->stmt_init();
    if($stmt = $mysqli->prepare("INSERT INTO $usertable (toolkind,brand,powerkind,color,
	    hascase,price,descript,location_id,user_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)"))
    {
        // Bind parameters. Types: s=string, i=integer, d=double, etc.
		// protects against sql injections
        $stmt->bind_param('sssssssii', $toolkind, $brand, $powerkind, $color, $hascase, 
...
{
	global $toolkind, $brand, $powerkind, $color, $hascase, $price, $descript, $location_id, $user_id, $usertable;
	$index = $_POST['uid'];  // "uid" is id of db record to be updated 
    
    $stmt = $mysqli->stmt_init();
    if($stmt = $mysqli->prepare("UPDATE $usertable SET toolkind = ?, brand = ?, 
	    powerkind=?, color=?, hascase=?, price=?, descript=?, location_id=?, user_id=? WHERE id = ?"))
    {
        // Bind parameters. Types: s=string, i=integer, d=double, etc.
		// protects against sql injections
        $stmt->bind_param('sssssssiii', $toolkind, $brand, $powerkind, $color, $hascase, 
			...
			echo '	<br>
					...
					<form name="basic" method="POST" action="table05.php">
						<table class="table table-condensed" style="border: 1px solid #dddddd; border-radius: 5px; box-shadow: 2px 2px 10px;">
							...
							<tr><td>Tool Type: </td><td><input type="edit" name="toolkind" value="'. $row[1] .'" size="50" readonly></td></tr>
							<tr><td>Brand: </td><td><input type="edit" name="brand" value="' . $row[2] . '" size="50" readonly></td></tr>
							<tr><td>Power Type: </td><td><input type="edit" name="powerkind" value="' . $row[3] . '" size="50" readonly></td></tr>
							<tr><td>Color: </td><td><input type="edit" name="color" value="' . $row[4] . '" size="50" readonly></td></tr>
							<tr><td>Case Included: </td><td><input type="edit" name="hascase" value="' . $row[5] . '" size="50" readonly></td></tr>
							<tr><td>Price: </td><td><input type="edit" name="price" value="' . $row[6] . '" size="20" readonly></td></tr>
							<tr><td>Description: </td><td><input type="edit" name="descript" value="' . $row[7] . '" size="20" readonly></td></tr>
							<tr><td>Location: </td><td><input type="edit" name="location_id" value="' . $row[8] . '" size="20" readonly></td></tr>
							<tr><td>User: </td><td><input type="edit" name="user_id" value="' . $row[9] . '" size="20" readonly></td></tr>
