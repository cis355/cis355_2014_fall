<?php
# --------------------------------------------------------------------------- #
# program: table03a.php
# author:  george corser
# course:  cis355 fall 2014
# purpose: template for cis355
#          to update this template, change all instances of "table03a.php" to your php filename
...
function createTable($mysqli)
{
    ...
    if(!$id)
    {
	    $sql = "CREATE TABLE table03a (id INT NOT NULL AUTO_INCREMENT,PRIMARY KEY( id ),";
...
function showList($mysqli, $msg)
{
	...
	echo 	'<div class="col-md-12">
			<form action="table03a.php" method="POST">
			...
			<h2 style="color: white;">Guitar Database (table03a)</h2></td></tr>
			<tr style="font-weight:800; font-size:20px;"><td>ID #</td><td>Category</td><td>Brand</td>
			<td>Quality</td><td>Body Type</td><td>Fretboard</td><td>Cutaway</td><td>Price</td><td></td></tr>';
...
function showInsertForm ()
{
	...
	<form name="basic" method="POST" action="table03a.php" onSubmit="return validate();">
		<table class="table table-condensed" style="border: 1px solid #dddddd; border-radius: 5px; box-shadow: 2px 2px 10px;">
			<tr><td colspan="2" style="text-align: center; border-radius: 5px; color: white; background-color:#333333;">
			<h2>Guitar Form</h2></td></tr>
			...
			<tr><td>User: </td><td><input type="edit" name="user_id" value="$" size="10"></td></tr>
		...
		</table><a href="table03a.php" class="btn btn-primary">Display Database</a></form></div>';
}
...
function showUpdateForm($mysqli) 
{
	...
	if($result = $mysqli->query("SELECT * FROM $usertable WHERE id = $index"))
	{
		while($row = $result->fetch_row())
		{
			echo '	<br>
					<div class="col-md-4">
					<form name="basic" method="POST" action="table03a.php">
						<table class="table table-condensed" style="border: 1px solid #dddddd; border-radius: 5px; box-shadow: 2px 2px 10px;">
...
{
    global $category, $brand, $quality, $bodytype, $fretboard, $cutaway, $price, $usertable;
	
    $stmt = $mysqli->stmt_init();
    if($stmt = $mysqli->prepare("INSERT INTO $usertable (category,brand,quality,bodytype,fretboard,cutaway,price) VALUES (?, ?, ?, ?, ?, ?, ?)"))
    {
        // Bind parameters. Types: s = string, i = integer, d = double,  b = blob, etc.
		// bind_param replaces question mark(s) with contents of variable(s) to protect against sql injection
        $stmt->bind_param('sssssss', $category, $brand, $quality, $bodytype, $fretboard, $cutaway, $price);
        $stmt->execute();
        $stmt->close();
    }
...
function updateRecord($mysqli)
{
    ...
    $stmt = $mysqli->stmt_init();
    if($stmt = $mysqli->prepare("UPDATE $usertable SET category = ?, brand = ?, quality = ?, bodytype = ?, fretboard = ?, cutaway = ?, price = ? WHERE id = ?"))
    {
            // Bind parameters. Types: s = string, i = integer, d = double,  b = blob, etc.
			// bind_param replaces question smark(s) with contents of variable(s) to protect against sql injection
            $stmt->bind_param('ssssssss', $category, $brand, $quality, $bodytype, $fretboard, $cutaway, $price, $index);
            $stmt->execute();
            $stmt->close();
    }
}
