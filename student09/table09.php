...
if($mysqli)            
{
    ...
    $year 				= $_POST['year']; // if does not exist then value is ""
	$make 				= $_POST['make'];
	$model 				= $_POST['model'];
...
echo '<p><a href="bio.html">Student BIO</a></p>';
...
function showList($mysqli, $msg) 
{
	...
	echo 	'<div class="col-md-12">
			...
			<h2 style="color: white;">Table 01</h2>
			</td></tr><tr style="font-weight:800; font-size:20px;">
			<td>ID</td><td>Year</td><td>Make</td>
			<td>Model</td><td>Condition</td><td>Price</td>						
...
function showInsertForm ($mysqli)
{
	...
	<form name="basic" method="POST" action="table09.php" 
		...
		<table class="table table-condensed" style="border: 1px solid #dddddd; 
			...
			size="20"></td></tr>
			<tr><td>Make: </td><td><input type="edit" name="make" value="" 
			size="20"></td></tr>
			<tr><td>Model: </td><td><input type="edit" name="model" value="" 
			size="30"></td></tr>
			<tr><td>Condition: </td><td><input type="edit" name="vcondition" value="" 
			size="20"></td></tr>
			<tr><td>Price: </td><td><input type="edit" name="price_paid" value="" 
			size="20"></td></tr>';
/*			
...
*/	
			...
			echo '</td></tr>
		        ...
		        </table><a href="table09.php" class="btn btn-primary">
			...
			echo '	<br>
					...
					<form name="basic" method="POST" action="table09.php">
						<table class="table table-condensed" 
							...
							<h2>Vehicle Form</h2></td></tr>
							<tr><td>Year: </td><td><input type="edit" 
							name="year" value="'. $row[1] .'" size="20">
							</td></tr>
							<tr><td>Make: </td><td><input type="edit" 
							name="make" value="' . $row[2] . '" size="20">
							</td></tr>
							<tr><td>Model: </td><td><input type="edit" 
							name="model" value="' . $row[3] . '" size="30">
							</td></tr>
							<tr><td>Condition: </td><td><input type="edit" 
							name="vcondition" value="' . $row[4] . '" size="20">
							</td></tr>
							<tr><td>Price: </td><td><input type="edit" 
							name="price_paid" value="' . $row[5] . '" size="20">
							</td></tr>';	
/*							
							<tr><td>Location ID: </td><td><textarea 							
							style="resize: none;" name="location_id" cols="40" 							
...
{
    global $year, $make, $model, $vcondition, $price_paid, $location_id, $user_id ;
	global $usertable;
    
    $stmt = $mysqli->stmt_init();
    if($stmt = $mysqli->prepare("INSERT INTO $usertable (year,make,model,vcondition,price_paid,
    		location_id,user_id) VALUES (?, ?, ?, ?, ?, ?, ?)"))
    {
        // Bind parameters. Types: s=string, i=integer, d=double, etc.
		// protects against sql injections
        $stmt->bind_param('sssssii', $year, $make, $model, $vcondition, 
		    $price_paid, $location_id, $user_id);
        $stmt->execute();
        $stmt->close();
    }
}
...
function updateRecord($mysqli)
{
    ...
    $stmt = $mysqli->stmt_init();
    if($stmt = $mysqli->prepare("UPDATE $usertable SET year = ?, make = ?, 
	    model=?, vcondition=?, price_paid=?, location_id=?, user_id=? WHERE id = ?"))
    {
        // Bind parameters. Types: s=string, i=integer, d=double, etc.
		// protects against sql injections
        $stmt->bind_param('sssssiii', $year, $make, $model, $vcondition, 
		    $price_paid, $location_id, $user_id, $index);
        $stmt->execute();
        $stmt->close();
    }
}
