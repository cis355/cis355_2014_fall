<?php
# --------------------------------------------------------------------------- #
# program: table09b.php 
# author:  george corser
# course:  cis355 fall 2014
# edited:  Trevin Damm / 2014 10 06
# purpose: template for cis355
#          to update this template, change all instances of "table09b.php" to your php filename
...
function createTable($mysqli)
{
    ...
    if(!$id)
    {
	    $sql = "CREATE TABLE table09b (id INT NOT NULL AUTO_INCREMENT,PRIMARY KEY( id ),";
...
function showList($mysqli, $msg)
{
	...
	echo 	'<div class="col-md-12">
			<form action="table09b.php" method="POST">
			...
			<h2 style="color: white;">Vehicle Database (table09b)</h2></td></tr>
			<tr style="font-weight:800; font-size:20px;"><td>ID #</td><td>user_id</td><td>location_id</td>
			<td>year</td><td>make:</td><td>String/Wind</td><td>condition</td><td>price_paidion</td><td></td></tr>';
...
function showInsertForm ()
{
	...
	<form name="basic" method="POST" action="table09b.php" onSubmit="return validate();">
		<table class="table table-condensed" style="border: 1px solid #dddddd; border-radius: 5px; box-shadow: 2px 2px 10px;">
			...
			<tr><td>String/Wind: </td><td><input type="edit" name="model" value="" size="30"></td></tr>
			<tr><td>condition: </td><td><input type="edit" name="condition" value="" size="20"></td></tr>
			<tr><td>price_paidion: </td><td><textarea style="resize: none;" name="price_paid" cols="40" rows="3"></textarea></td></tr>
		...
		</table><a href="table09b.php" class="btn btn-primary">Display Database</a></form></div>';
}
...
function showUpdateForm($mysqli) 
{
	...
	if($result = $mysqli->query("SELECT * FROM $usertable WHERE id = $index"))
	{
		while($row = $result->fetch_row())
		{
			echo '	<br>
					<div class="col-md-4">
					<form name="basic" method="POST" action="table09b.php">
						<table class="table table-condensed" style="border: 1px solid #dddddd; border-radius: 5px; box-shadow: 2px 2px 10px;">
							...
							<tr><td>location_id: </td><td><input type="edit" name="location_id" value="' . $row[2] . '" size="20"></td></tr>
							...
							<tr><td>String/Wind: </td><td><input type="edit" name="model" value="' . $row[5] . '" size="30"></td></tr>
							<tr><td>condition: </td><td><input type="edit" name="condition" value="' . $row[6] . '" size="20"></td></tr>
							<tr><td>price_paidion: </td><td><textarea style="resize: none;" name="price_paid" cols="40" rows="3">' . $row[7] . '</textarea></td></tr>
...
{
    global $user_id, $location_id, $year, $make, $model, $condition, $price_paid, $usertable;
    
    $stmt = $mysqli->stmt_init();
    if($stmt = $mysqli->prepare("INSERT INTO $usertable (user_id,location_id,year,make,model,condition,price_paid) VALUES (?, ?, ?, ?, ?, ?, ?)"))
    {
        // Bind parameters. Types: s = string, i = integer, d = double,  b = blob, etc.
		// bind_param replaces question mark(s) with contents of variable(s) to protect against sql injection
        $stmt->bind_param('sssssss', $user_id, $location_id, $year, $make, $model, $condition, $price_paid);
        $stmt->execute();
        $stmt->close();
    }
}
...
function updateRecord($mysqli)
{
    ...
    $stmt = $mysqli->stmt_init();
    if($stmt = $mysqli->prepare("UPDATE $usertable SET user_id = ?, location_id = ?, year = ?, make = ?, model = ?, condition = ?, price_paid = ? WHERE id = ?"))
    {
            // Bind parameters. Types: s = string, i = integer, d = double,  b = blob, etc.
			// bind_param replaces question mark(s) with contents of variable(s) to protect against sql injection
            $stmt->bind_param('sssssssi', $user_id, $location_id, $year, $make, $model, $condition, $price_paid, $index);
            $stmt->execute();
            $stmt->close();
    }
}
