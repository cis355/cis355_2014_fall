...
#          to update this template, change all instances of "table18.php" to your php filename
...
function createTable($mysqli)
{
    ...
    if(!$id)
    {
		...
		$sql .= "FOREIGN KEY (loc_id) REFERENCES location18(loc_id)";
	    $sql .= ")";
...
function showList($mysqli, $msg)
{
	...
	echo 	'<div class="col-md-12">
			...
			<td>Model</td><td>Price</td><td>Description</td><td></td></tr>';
			...
			echo '	<br>
					...
					<form name="basic" method="POST" action="table18.php">
						<table class="table table-condensed" style="border: 1px solid #dddddd; border-radius: 5px; box-shadow: 2px 2px 10px;">
							...
							<tr><td>User_ID: </td><td><input type="edit" name="user_id" value="' . $row[6] . '" size="20"></td></tr>
							<tr><td>Loc_ID: </td><td><input type="edit" name="loc_id" value="' . $row[7] . '" size="20"></td></tr>
...
{
    global $type, $brand, $model, $price, $descript, $usertable;
    
    $stmt = $mysqli->stmt_init();
    if($stmt = $mysqli->prepare("INSERT INTO $usertable (type,brand,model,price,descript,user_ID,loc_ID) VALUES (?, ?, ?, ?, ?, ?, ?)"))
    {
        // Bind parameters. Types: s = string, i = integer, d = double,  b = blob, etc.
		// bind_param replaces question mark(s) with contents of variable(s) to protect against sql injection
        $stmt->bind_param('sssssii', $type, $brand, $model, $price, $descript, $user_ID, $loc_ID);
        $stmt->execute();
        $stmt->close();
    }
}
...
function updateRecord($mysqli)
{
    ...
    $stmt = $mysqli->stmt_init();
    if($stmt = $mysqli->prepare("UPDATE $usertable SET type = ?, brand = ?, model = ?, price = ?, descript = ?, user_ID = ?, loc_ID = ? WHERE id = ?"))
    {
            // Bind parameters. Types: s = string, i = integer, d = double,  b = blob, etc.
			// bind_param replaces question mark(s) with contents of variable(s) to protect against sql injection
            $stmt->bind_param('sssssiii', $type, $brand, $model, $price, $descript, $user_ID, $loc_ID, $index);
            $stmt->execute();
            $stmt->close();
    }
}
