...
else
{
    ...
    <tr><td width=\"35%\">description</td><td>$description</td></tr>
	<tr><td width=\"35%\">year</td><td>$year</td></tr>
	<tr><td width=\"35%\">cost</td><td>$cost</td></tr>
    </table></td></tr></table>";
...
function insertRecord($mysqli)
{
    ...
    global $name, $description, $year, $cost, $usertable;
    
    /* Initialise the statement. */
    $stmt = $mysqli->stmt_init();
    /* Notice the two ? in values, these will be bound parameters*/
    if($stmt = $mysqli->prepare("INSERT INTO $usertable (name,description,year,cost) VALUES (?, ?, ?, ?)"))
    {
            /* Bind parameters. Types: s = string, i = integer, d = double,  b = blob, etc... */
            $stmt->bind_param('ssid', $name, $description, $year, $cost);
...
function updateRecord($mysqli)
{
    ...
    $sql = "UPDATE $usertable SET description = $description, year = $year, cost = $cost WHERE name = $name";
...
function selectRecords($mysqli)
{
    ...
    if($stmt = $mysqli->prepare("select * from $usertable"))
    {
        ...
        $stmt->execute();
        /* bind result vars */
        $stmt->bind_result($id, $name, $description, $year, $cost);
        /* read while fetch returns data*/
        echo "<br> Data selected from the MySQL table.<table><tr><th>ID</th><th>Name</th><th>description</th><th>year</th><th>cost</th></tr>";
        while($stmt->fetch()){
                /* work with bound result vars */
                echo "<tr><td>$id</td><td>$name</td><td>$description</td><td>$year</td><td>$cost</td></tr>";
        }
        echo "</table>";
        $stmt->close();
    }
...
function showForm()
{
    return "<!DOCTYPE html><html>
		...
		<form name='basic' method='Post' action='tempCIS355student17.php' onSubmit='return validate();'>
        <table border=0 cellpadding=5 cellspacing=0>
        <tr><td>name</td><td><input type='edit' name='name' value='' size=20></td></tr>
		<tr><td>description</td><td><input type='edit' name='description' value='' size='30'></td></tr>        
		<tr><td>year</td><td><input type='edit' name='year' value='' size='10'></td></tr>
		<tr><td>cost</td><td><input type='edit' name='cost' value='' size='10'></td></tr>
...
{
    echo "<br><form name='basic' method='Post' action='tempCIS355student17.php' onSubmit='return validate();'>
