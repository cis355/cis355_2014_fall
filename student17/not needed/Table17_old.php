    ...
    echo   '<div class="col-md-4">
			<form name="basic" method="POST" action="Table17.php" onSubmit="return validate();">
				<table class="table table-condensed" style="border: 1px solid #dddddd; border-radius: 5px; box-shadow: 2px 2px 10px;">
					...
					<tr><td>User:</td><td><input type="edit" name="user_id" value="" size="5"></td></tr>
					<tr><td>Location:</td><td><input type="edit" name="location_id" value="" size="5"></td></tr>
			...
			echo '	<br>
					...
					<form name="basic" method="POST" action="Table17.php">
						<table class="table table-condensed" style="border: 1px solid #dddddd; border-radius: 5px; box-shadow: 2px 2px 10px;">
							...
							<tr><td>User:</td><td><input type="edit" name="user_id" value="'. $row[10] .'" size="5"></td></tr>
							<tr><td>Location:</td><td><input type="edit" name="location_id" value="'. $row[11] .'" size="5"></td></tr>
...
{
    global $itemName, $manufacturer, $year, $doorStyle, $itemCondition, $defrostType, $color, $amountPaid, $descript, $userID, $locationID, $usertable;
    
    $stmt = $mysqli->stmt_init();
    if($stmt = $mysqli->prepare("INSERT INTO $usertable (itemName, manufacturer, year, doorStyle, itemCondition, defrostType, color, amountPaid, descript, user_id, location_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"))
    {
        // Bind parameters. Types: s = string, i = integer, d = double,  b = blob, etc.
		// bind_param replaces question mark(s) with contents of variable(s) to protect against sql injection
        $stmt->bind_param('sssssssssii', $itemName, $manufacturer, $year, $doorStyle, $itemCondition, $defrostType, $color, $amountPaid, $descript, $userID, $locationID);
        $stmt->execute();
        $stmt->close();
    }
}
...
function updateRecord($mysqli)
{
    ...
    $stmt = $mysqli->stmt_init();
    if($stmt = $mysqli->prepare("UPDATE $usertable SET itemName = ?, manufacturer = ?, year = ?, doorStyle = ?, itemCondition = ?, defrostType = ?, color = ?, amountPaid = ?, descript = ?, user_id = ?, location_id = ?, WHERE id = ?"))
    {
            // Bind parameters. Types: s = string, i = integer, d = double,  b = blob, etc.
			// bind_param replaces question mark(s) with contents of variable(s) to protect against sql injection
            $stmt->bind_param('sssssssssiii', $itemName, $manufacturer, $year, $doorStyle, $itemCondition, $defrostType, $color, $amountPaid, $descript, $userID, $locationID, $index);
            $stmt->execute();
            $stmt->close();
    }
}
