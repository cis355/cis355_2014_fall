...
if($mysqli)            // if successful connection...
{
    ...
    $seller_id			= $_POST['seller_id']; // if does not exist then value is ""
	$sell_name			= $_POST['sell_name'];
	$sell_loc			= $_POST['sell_loc'];
	$sell_email			= $_POST['sell_email'];
	$pet_type			= $_POST['pet_type'];
	$pet_breed			= $_POST['pet_breed'];
	$descript 			= $_POST['descript'];
	$price				= $_POST['price'];
	$home_visit_info	=  $_POST['home_visit_info'];
...
function createTable($mysqli)
{
    ...
    if(!$id)
    {
	    $sql = "CREATE TABLE seller04 (id INT NOT NULL AUTO_INCREMENT,PRIMARY KEY(id),";
	    $sql .= "seller_id INT,";
	    $sql .= "sell_name VARCHAR(25),";
	    $sql .= "sell_loc VARCHAR(25),";
	    $sql .= "sell_email VARCHAR(25),";
	    $sql .= "pet_type VARCHAR(25),";
	    $sql .= "pet_breed VARCHAR(25),";
	    $sql .= "descript VARCHAR(300),";
		$sql .= "price FLOAT(6,2),";
		$sql .= "home_visit_info VARCHAR(100),";
		$sql .= "FOREIGN KEY(seller_id) REFERENCES userinfo04(user_id)";
	    $sql .= ")";
...
function showList($mysqli, $msg)
{
	...
	echo 	'<div class="col-md-12">
			...
			<h2 style="color: white;">Pets for Sale/Adoption (table04)</h2></td></tr>
			<tr style="font-weight:800; font-size:20px;"><td>ID #</td><td>Seller Name</td>
			<td>Location</td><td>Email</td><td>Pet Type</td><td>Breed</td><td>Price</td><td></td></tr>';
...
function showInsertForm ($mysqli)
{
	...
	<form name="basic" method="POST" action="sellers.php" onSubmit="return validate();">
		<table class="table table-condensed" style="border: 1px solid #dddddd; border-radius: 5px; box-shadow: 2px 2px 10px;">
			...
			<tr><td>Name: </td><td><input type="edit" name="sell_name" value="" size="25"></td></tr>
			<tr><td>Location: </td><td><input type="edit" name="sell_loc" value="" size="25"></td></tr>
			<tr><td>Email: </td><td><input type="edit" name="sell_email" value="" size="25"></td></tr>
			<tr><td>Pet Type: </td><td><input type="edit" name="pet_type" value="" size="15"></td></tr>
			<tr><td>Breed: </td><td><input type="edit" name="pet_breed" value="" size="15"></td></tr>
			<tr><td>Description: </td><td><textarea style="resize: none;" name="descript" cols="40" rows="3"></textarea></td></tr>';
	echo
			'<tr><td>User ID: </td><td><input type="text" name="seller_id" value="'. $_SESSION["id"].'" size="10" readonly></td></tr>
			<tr><td>Price (if free, put 0): </td><td><input type="edit" name="price" value="" size="8"></td></tr>
			<tr><td>Home visits? (Yes/No/What Times): </td><td><textarea style="resize: none;" name="home_visit_info" cols="40" rows="3"></textarea></td></tr>
		...
		</table><a href="sellers.php" class="btn btn-primary">Display Database</a></form></div>';
}
...
function showUpdateForm($mysqli) 
{
	...
	if($result = $mysqli->query("SELECT * FROM $usertable WHERE id = $index"))
	{
		while($row = $result->fetch_row())
		{
			echo '	<br>
					<div class="col-md-4">
					<form name="basic" method="POST" action="sellers.php">
						<table class="table table-condensed" style="border: 1px solid #dddddd; border-radius: 5px; box-shadow: 2px 2px 10px;">
							...
							<tr><td>Name: </td><td><input type="edit" name="sell_name" value="'. $row[2] .'" size="20"></td></tr>
							<tr><td>Location: </td><td><input type="edit" name="sell_loc" value="' . $row[3] . '" size="20"></td></tr>
							<tr><td>Email: </td><td><input type="edit" name="sell_email" value="' . $row[4] . '" size="30"></td></tr>
							<tr><td>Pet Type: </td><td><input type="edit" name="pet_type" value="' . $row[5] . '" size="20"></td></tr>
							<tr><td>Breed: </td><td><input type="edit" name="pet_breed" value="' . $row[6] . '" size="30"></td></tr>
							<tr><td>Description: </td><td><textarea style="resize: none;" name="descript" cols="40" rows="3">' . $row[7] . '</textarea></td></tr>
							<tr><td>Price: </td><td><input type="edit" name="price" value="' . $row[8] . '" size="8"></td></tr>
							<tr><td>Home visit? (Yes/No/What Times): </td><td><textarea style="resize: none;" name="home_visit_info" cols="40" rows="3">' . $row[9] . '</textarea></td></tr>';
					echo
							'</td></tr>
							<tr><td>User ID: </td><td><input type="edit" name="seller_id" value="' . $row[1] . '" size="20" readonly></td></tr>
...
{
    global $seller_id, $sell_name, $sell_loc, $sell_email, $pet_type, $pet_breed, $descript, $price, $home_visit_info, $usertable;

    $stmt = $mysqli->stmt_init();
    if($stmt = $mysqli->prepare("INSERT INTO $usertable (seller_id,sell_name,sell_loc,sell_email,pet_type,pet_breed,descript,price,home_visit_info) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)"))
    {
        // Bind parameters. Types: s = string, i = integer, d = double,  b = blob, etc.
		// bind_param replaces question mark(s) with contents of variable(s) to protect against sql injection
        $stmt->bind_param('issssssss', $seller_id, $sell_name, $sell_loc, $sell_email, $pet_type, $pet_breed, $descript, $price, $home_visit_info);
        $stmt->execute();
        $stmt->close();
    }
...
{
	global $sell_name, $sell_loc, $sell_email, $pet_type, $pet_breed, $descript, $price, $home_visit_info, $usertable;
	$index = $_POST['uid'];  // "uid" is id of db record to be updated (from the form)
    
    $stmt = $mysqli->stmt_init();
    if($stmt = $mysqli->prepare("UPDATE $usertable SET sell_name=?, sell_loc=?, sell_email=?, pet_type=?, pet_breed=?, descript=?, price=?, home_visit_info=? WHERE id = ?"))
    {
            // Bind parameters. Types: s = string, i = integer, d = double,  b = blob, etc.
			// bind_param replaces question mark(s) with contents of variable(s) to protect against sql injection
            $stmt->bind_param('ssssssssi', $sell_name, $sell_loc, $sell_email, $pet_type, $pet_breed, $descript, $price, $home_visit_info, $index);
            $stmt->execute();
            $stmt->close();
    }
}
	...
	if($result = $mysqli->query("SELECT * FROM $usertable WHERE id = $index"))
	{
	    $row = $result->fetch_row();
		 echo '	<br>
				...
				<h2>Pets for Sale/Adoption:</h2></td></tr>
				<tr><td>Seller Id: </td><td>'. $row[1] .'</td></tr>
				<tr><td>Seller Name: </td><td>' . $row[2] . '</td></tr>
				...
				<tr><td>Price: </td><td>' . $row[8] . '</td></tr>
				<tr><td>Home Visiting Information: </td><td>' . $row[9] . '</td></tr>
				</table>
                <a href="sellers.php" class="btn btn-primary">Display Database</a>
				</form></div>';
		$result->close();
	}
}
