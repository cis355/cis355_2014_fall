<DOCTYPE HTML><html>
<head><title>LWIP Compound Bow Olivia Archambo</title></head>
<body>
<h1>LWIP: Compound Bows</h1>
</body>
</html>

<?php
/**
 *http://cis355.com/student06/etc/tables.php
 *
...
else
{
	...
	<tr><td width=\"35%\">User ID</td><td>$user_id</td></tr>
	<tr><td width=\"35%\">Location ID</td><td>$loc_id</td></tr>
    <tr><td width=\"35%\">Name</td><td>$name</td></tr>
    <tr><td width=\"35%\">Description</td><td>$descript</td></tr>
	<tr><td width=\"35%\">Amount Paid</td><td>$amount_paid</td></tr>
	<tr><td width=\"35%\">Draw Weight</td><td>$drweight</td></tr>
	<tr><td width=\"35%\">Draw Length</td><td>$drlength</td></tr>
	<tr><td width=\"35%\">Condition</td><td>$cond</td></tr>
    </table></td></tr></table>";
...
function insertRecord($mysqli)
{
    ...
    global $user_id, $loc_id, $name, $descript, $amount_paid, $drweight, $drlength, $cond, $usertable;
    
    /* Initialise the statement. */
    $stmt = $mysqli->stmt_init();
    /* Notice the two ? in values, these will be bound parameters*/
    if($stmt = $mysqli->prepare("INSERT INTO $usertable (user_id, loc_id, name, descript, amount_paid, drweight, drlength, cond) VALUES (?, ?, ? ,?, ?, ?, ?, ?)"))
    {
            /* Bind parameters. Types: s = string, i = integer, d = double,  b = blob, etc... */
            $stmt->bind_param('iissdiis', $user_id,$loc_id,$name,$descript,$amount_paid,$drweight,$drlength,$cond);
...
function updateRecord($mysqli)
{
    ...
    $sql = "UPDATE $usertable SET descript='updated!' WHERE name=?";
...
function selectRecords($mysqli)
{
    ...
    if($stmt = $mysqli->prepare("select * from $usertable"))
    {
        ...
        $stmt->execute();
        /* bind result vars */
        $stmt->bind_result($id, $user_id, $loc_id, $name, $descript, $amount_paid, $drweight, $drlength, $cond);
        /* read while fetch returns data*/
        echo "<br> Data selected from the MySQL table.<table><tr><th>ID</th><th>User ID</th><th>Location ID</th><th>Name</th><th>Description</th><th>Amount Paid</th><th>Draw Weight</th><th>Draw Length</th><th>Condition</th></tr>";
        while($stmt->fetch()){
                /* work with bound result vars */
                echo "<tr><td>$id</td><td>$user_id</td><td>$loc_id</td><td>$name</td><td>$descript</td><td>$amount_paid</td><td>$drweight</td><td>$drlength</td><td>$cond</td></tr>";
        }
        echo "</table>";
        $stmt->close();
    }
...
function showForm()
{
    return "<!DOCTYPE html><html>
        <head><title>Olivia Archambo table04</title></head><body><style>
        ...
        <script language='Javascript'>
         ...
         <form name='basic' method='Post' action='table04a.php' onSubmit='return validate();'>
        <table border=0 cellpadding=5 cellspacing=0>
		<tr><td>User Id:</td><td><input type='edit' name='user_id' value='' size=6></td></tr>
		<tr><td>Location Id/ZIP Code:</td><td><input type='edit' name='loc_id' value='' size=5></td></tr>
		...
		<tr><td>Amount Paid:</td><td><input type='edit' name='amount_paid' value='' size=6></td></tr>
...
{
    echo "<br><form name='basic' method='Post' action='table04a.php' onSubmit='return validate();'>
