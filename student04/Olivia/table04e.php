<?php
# --------------------------------------------------------------------------- #
# program: table04e.php
# author:  george corser
# course:  cis355 fall 2014
# purpose: template for cis355
#          to update this template, change all instances of "table04e.php" to your php filename
...
function createTable($mysqli)
{
    ...
    if(!$id)
    {
	    $sql = "CREATE TABLE table04e (id INT NOT NULL AUTO_INCREMENT,PRIMARY KEY( id ),";
	    $sql .= "type VARCHAR(20),";
...
function showList($mysqli, $msg)
{
	...
	echo 	'<div class="col-md-12">
			<form action="table04e.php" method="POST">
			...
			<h2 style="color: white;">Compound Bow Database (table04e)</h2></td></tr>
...
function showInsertForm ()
{
	...
	<form name="basic" method="POST" action="table04e.php" onSubmit="return validate();">
		<table class="table table-condensed" style="border: 1px solid #dddddd; border-radius: 5px; box-shadow: 2px 2px 10px;">
		...
		</table><a href="table04e.php" class="btn btn-primary">Display Database</a></form></div>';
}
...
function showUpdateForm($mysqli) 
{
	...
	if($result = $mysqli->query("SELECT * FROM $usertable WHERE id = $index"))
	{
		while($row = $result->fetch_row())
		{
			echo '	<br>
					<div class="col-md-4">
					<form name="basic" method="POST" action="table04e.php">
						<table class="table table-condensed" style="border: 1px solid #dddddd; border-radius: 5px; box-shadow: 2px 2px 10px;">
