...
else
{
    ...
    <tr><td width=\"35%\">descript</td><td>$descript</td></tr>
    </table></td></tr></table>";
...
function insertRecord($mysqli)
{
    ...
    global $name, $descript, $user_id, $loc_id, $amount_paid, $drweight, $drlength, $cond $usertable;
...
function selectRecords($mysqli)
{
    ...
    if($stmt = $mysqli->prepare("select * from $usertable"))
    {
        ...
        echo "<br> Data selected from the MySQL table.<table><tr><th>ID</th><th>Name</th><th>Description</th></tr>";
        while($stmt->fetch()){
                /* work with bound result vars */
                echo "<tr><td>$id</td><td>$name</td><td>$descript</td></tr>";
        }
        echo "</table>";
        $stmt->close();
    }
...
function showForm()
{
    return "<!DOCTYPE html><html>
        ...
        <form name='basic' method='Post' action='table04b.php' onSubmit='return validate();'>
...
{
    echo "<br><form name='basic' method='Post' action='table04b.php' onSubmit='return validate();'>
