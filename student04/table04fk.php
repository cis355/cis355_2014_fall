<?php
# --------------------------------------------------------------------------- #
# program: table04fk.php
# author:  george corser
# editor:  Olivia Archambo
# course:  cis355 fall 2014
# purpose: template for cis355
#          to update this template, change all instances of "table04fk.php" to your php filename
...
function createTable($mysqli)
{
    ...
    if(!$id)
    {
		...
		$sql .= "loc_id INT,";
		$sql .= "user_id INT,";
		$sql .= "FOREIGN KEY(user_id) REFERENCES user04(user_id),";
		$sql .= "FOREIGN KEY(loc_id) REFERENCES location04(loc_id)";
	    $sql .= ")";
...
function showList($mysqli, $msg)
{
	...
	echo 	'<div class="col-md-12">
			<form action="table04fk.php" method="POST">
			...
			<td>Condition (New/Used):</td><td>Draw Weight:</td><td>Draw Length:</td><td>Price</td><td>Description</td><td></td></tr>';
...
function showInsertForm ()
{
	...
	<form name="basic" method="POST" action="table04fk.php" onSubmit="return validate();">
		<table class="table table-condensed" style="border: 1px solid #dddddd; border-radius: 5px; box-shadow: 2px 2px 10px;">
			...
			<tr><td>Draw Weight: </td><td><input type="edit" name="drweight" value="" size="20"></td></tr>
			<tr><td>Draw Length: </td><td><input type="edit" name="drlength" value="" size="30"></td></tr>
		...
		</table><a href="table04fk.php" class="btn btn-primary">Display Database</a></form></div>';
}
...
function showUpdateForm($mysqli) 
{
	...
	if($result = $mysqli->query("SELECT * FROM $usertable WHERE id = $index"))
	{
		while($row = $result->fetch_row())
		{
			echo '	<br>
					<div class="col-md-4">
					<form name="basic" method="POST" action="table04fk.php">
						<table class="table table-condensed" style="border: 1px solid #dddddd; border-radius: 5px; box-shadow: 2px 2px 10px;">
							...
							<tr><td>Condition (New/Used): </td><td><input type="edit" name="cond" value="' . $row[3] . '" size="30"></td></tr>
...
{
    global $type, $brand, $cond, $drweight, $drlength, $price, $descript, $usertable;
    
    $stmt = $mysqli->stmt_init();
    if($stmt = $mysqli->prepare("INSERT INTO $usertable (type,brand,cond,drweight,drlength,price,descript) VALUES (?, ?, ?, ?, ?, ?, ?)"))
    {
        // Bind parameters. Types: s = string, i = integer, d = double,  b = blob, etc.
		// bind_param replaces question mark(s) with contents of variable(s) to protect against sql injection
        $stmt->bind_param('sssssss', $type, $brand, $cond, $drweight, $drlength, $price, $descript);
        $stmt->execute();
        $stmt->close();
    }
...
function updateRecord($mysqli)
{
    ...
    $stmt = $mysqli->stmt_init();
    if($stmt = $mysqli->prepare("UPDATE $usertable SET type = ?, brand = ?, cond = ?, drweight = ?, drlength = ?, price = ?, descript = ? WHERE id = ?"))
    {
            // Bind parameters. Types: s = string, i = integer, d = double,  b = blob, etc.
			// bind_param replaces question mark(s) with contents of variable(s) to protect against sql injection
            $stmt->bind_param('sssssssi', $type, $brand, $cond, $drweight, $drlength, $price, $descript, $index);
            $stmt->execute();
            $stmt->close();
    }
}
